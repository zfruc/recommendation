...
public class CreateItemData {
    ...
    public static void generate(int num) throws IOException {
        ...
        List<String> categories = Arrays.asList("Women's clothing", "Men's", "Women's shoes", "makeups", "Watch", "Mobile phone",
                "Maternal baby", "toy", "Snack", "Tea wine", "Fresh", "fruit", "Home appliance", "Furniture", "Building materials",
                "car", "Accessories", "Home textile", "medicine", "Kitchenware", "Storage");

        List<String> cites = Arrays.asList("beijing-beijing", "shanghai-shanghai", "guangzhou-guangdong", "shenzheng-shenzheng", "chengdu-sichuan", "hangzhou-zhejiang",
                "wuhan-hubei", "changsha-hunan", "kunming-yunnan", "xiamen-fujian", "zhengzhou-henan", "shijiazhuang-hebei");
        ...
        for (int i = 0; i < num; i++) {
            ...
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item_id"), Bytes.toBytes(String.valueOf(i)));

            String item_expo_id = String.valueOf(random.nextInt(100));
            itemFeature.put("item_expo_id", item_expo_id);
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item_expo_id"), Bytes.toBytes(item_expo_id));

            String item_category = categories.get(random.nextInt(categories.size()));
            itemFeature.put("item_category", item_category);
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item_category"), Bytes.toBytes(item_category));

            String item_category_level1 = String.valueOf(random.nextInt(4));
            itemFeature.put("item_category_level1", item_category_level1);
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item_category_level1"), Bytes.toBytes(item_category_level1));
            ...
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item_seller_city"), Bytes.toBytes(city.split("-")[0]));
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item_seller_prov"), Bytes.toBytes(city.split("-")[1]));

            String item_purch_level = String.valueOf(random.nextInt(5));
            itemFeature.put("item_purch_level", item_purch_level);
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item_purch_level"), Bytes.toBytes(item_purch_level));
            ...
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item_buyer_age"), Bytes.toBytes(item_buyer_age));

            String item_style_id = String.valueOf(random.nextInt(20));
            itemFeature.put("item_style_id", item_style_id);
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item_style_id"), Bytes.toBytes(item_style_id));

            String item_material_id = String.valueOf(random.nextInt(20));
            itemFeature.put("item_material_id", item_material_id);
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item_material_id"), Bytes.toBytes(item_material_id));

            String item_pay_class = String.valueOf(random.nextInt(5));
            itemFeature.put("item_pay_class", item_pay_class);
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item_pay_class"), Bytes.toBytes(item_pay_class));

            String item_brand_id = String.valueOf(random.nextInt(100));
            itemFeature.put("item_brand_id", item_brand_id);
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item_brand_id"), Bytes.toBytes(item_brand_id));
            ...
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item__i_shop_id_ctr"), Bytes.toBytes(item__i_shop_id_ctr));
            ...
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item__i_brand_id_ctr"), Bytes.toBytes(item__i_brand_id_ctr));
            ...
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("item__i_category_ctr"), Bytes.toBytes(item__i_category_ctr));
