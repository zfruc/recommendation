...
public class RecommendationLocal {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<String> kafkaSourceStream = streamEnv.addSource(
                new FlinkKafkaConsumer011<>(Constants.KAFKA_SOURCE_TOPIC, new SimpleStringSchema(), Util.getKafkaProPerties()));
        DataStream<Tuple2<String, List<String>>> itemsStream = kafkaSourceStream.map(new Recall()::recall);
        DataStream<Row> samples = itemsStream.map(tuple -> new Predict().generateSample(tuple.f0, tuple.f1)).returns(typeInfo);
        Table sampleTable = tableEnv.fromDataStream(samples);
        TFConfig tfConfig = createTFConfig(createSerializeConf());
        Table predictResult = TFUtils.inference(
                streamEnv,
                tableEnv,
                sampleTable,
                tfConfig,
                TypeUtil.rowTypeInfoToSchema(typeInfo));
